...
from acouchbase_analytics.credential import (
...
class CredentialTestSuite:
    ...
    async def test_dynamic_auth_picks_up_rotated_credential(self) -> None:
        ...
        await client.update_credential(JwtCredential(new_token))
    ...
    async def test_set_credential_password_to_jwt_fails(self) -> None:
        ...
        client = _AsyncClientAdapter('http://localhost', cred)
        with pytest.raises(ValueError):
            await client.update_credential(JwtCredential(_SAMPLE_JWT))
    ...
    async def test_set_credential_jwt_to_password_fails(self) -> None:
        ...
        client = _AsyncClientAdapter('http://localhost', cred)
        with pytest.raises(ValueError):
            await client.update_credential(Credential('Administrator', 'password'))
    ...
    @pytest.mark.asyncio
    async def test_set_credential_certificate_rotation_rebuilds_client(
        ...
        old_client.aclose = _spy_aclose  # type: ignore[method-assign]

        await client.update_credential(CertificateCredential(cert_path, key_path))

        assert client._client is not old_client
        assert closed == [True]

    @pytest.mark.asyncio
    async def test_set_credential_certificate_to_password_fails(self, cert_paths: Tuple[str, str]) -> None:
        cert_path, key_path = cert_paths
        cred = CertificateCredential(cert_path, key_path)
        client = _AsyncClientAdapter('https://localhost', cred)
        with pytest.raises(ValueError):
            await client.update_credential(Credential('u', 'p'))
